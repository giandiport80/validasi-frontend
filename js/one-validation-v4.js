...
function validateFormRules(selectorForm, rules, lang = "id") {
	...
	Object.keys(rules).forEach((selector) => {
		...
		config.rules.forEach((ruleStr) => {
			...
			switch (ruleName) {
				case "required":
					$el.data("required", true);
					break;
				case "alphabetic":
					$el.data("alphabetic", true);
					break;
				case "number":
					$el.data("isnumber", true);
					break;
				case "email":
					$el.data("email", true);
					break;
				case "no_html":
					...
					break;
				case "min":
					$el.data("minlength", parseInt(ruleValue));
					break;
				case "max":
					$el.data("maxlength", parseInt(ruleValue));
					break;
				case "min_val":
					$el.data("minvalue", parseFloat(ruleValue));
					break;
				case "max_val":
					$el.data("maxvalue", parseFloat(ruleValue));
					break;
				case "mimes":
					$el.data("extfile", ruleValue);
					break;
				case "size":
					$el.data("size", parseFloat(ruleValue));
					break;
				case "not_regex":
					$el.data("specialchar", ruleValue);
					break;
				case "size_type":
					$el.data("typesize", ruleValue);
					break;
			}
		});
		...
		if (type === "text") {
			...
			$el.addClass("validate-select");
		} else if (type === "radio") {
			$el.addClass("validate-radio");
		} else if (type === "file") {
			$el.addClass("validate-file");
		} else if (type === "checkbox") {
			$el.addClass("validate-checkbox");
		}

		let result = true;
		if (type === "text") result = validateOnInput($el[0], lang);
		else if (type === "select") result = validateOnSelect($el[0], lang);
		else if (type === "radio") result = validateOnRadio($el.attr("name"), lang);
		else if (type === "file") result = validateOnFile($el[0], lang);
		else if (type === "checkbox")
			result = validateOnCheckbox($el.attr("name"), lang);
