const messages = {
  id: {
    ...
    noHtml: '{label} tidak boleh mengandung tag HTML',
  },
  en: {
    ...
    noHtml: '{label} cannot contain HTML tags',
  },
};
...
function validationSelect(element, pesan) {
  const $el = $(element);
  const errorPlaceSelector = $el.data('errorplace');

  if ($el.hasClass('select2-hidden-accessible')) {
    const $select2Container = $el.next('.select2-container');

    // Memberikan border merah langsung ke elemen seleksi select2
    $select2Container.find('.select2-selection').css('border-color', '#dc3545');

    if (errorPlaceSelector) {
      $(errorPlaceSelector).html(getErrorMessage(pesan));
    } else {
      // Hapus error lama jika ada sebelum menambah yang baru
      $select2Container.next('.error').remove();
      $select2Container.after(getErrorMessage(pesan));
    }
  } else {
    // Untuk select standar (non-select2)
    $el.next('.error').remove();
    if (errorPlaceSelector) {
      $(errorPlaceSelector).html(getErrorMessage(pesan));
    } else {
      $el.after(getErrorMessage(pesan));
    }
...
function validationSelectRemove(element) {
  const $el = $(element);
  const errorPlace = $el.data('errorplace');
  ...
  if ($el.hasClass('select2-hidden-accessible')) {
    ...
    $select2Container.next('.error').remove();
    // Kembalikan warna border ke semula
...
function validateOnInput(element, lang) {
  ...
  const specialChars = $(element).data('specialchar')
    ...
    : [];
  const isNoHtml = $(element).data('nohtml');

  validationRemove(element);

  const label =
    $(element).data('label') || (lang === 'en' ? 'This field' : 'Kolom ini');
  const langMessages = messages[lang] || messages.id;
  ...
  if (isNoHtml && /<[^>]*>/g.test(value)) {
    validation(element, langMessages.noHtml.replace('{label}', label));
    return false;
  }
...
function validateOnSelect(element, lang) {
  ...
  const label =
    $el.data('label') || (lang === 'en' ? 'This field' : 'Kolom ini');
  const langMessages = messages[lang] || messages.id;
...
function validateOnRadio(name, lang) {
  ...
  const label =
    $(`input[name="${name}"]`).data('label') ||
    (lang === 'en' ? 'This field' : 'Kolom ini');
  const langMessages = messages[lang] || messages.id;
...
function validateOnFile(element, lang) {
  const $el = $(element);
  const required = $el.data('required');
  const allowedExtensions = $el.data('filetype');
  const typeSize = $el.data('typesize') || 'mb';
  const sizeLimit = parseFloat($el.data('size'));
  const file = element.files[0];
  const label = $el.data('label') || (lang === 'en' ? 'File' : 'File');
  const langMessages = messages[lang] || messages.id;

  validationRemove(element);

  if ((required === true || required === 'true') && !file) {
    validation(element, langMessages.required.replace('{label}', label));
    return false;
  }
  ...
  if (file) {
    ...
    const allowedArray = allowedExtensions
      ? allowedExtensions.split(',').map(ext => ext.trim())
      : [];

    if (allowedArray.length > 0 && !allowedArray.includes(fileExtension)) {
      const allowedExtensionText = allowedArray.join(', ');

      validation(
        element,
        langMessages.fileType
          .replace('{allowedExtensions}', allowedExtensionText)
          .replace('{label}', label),
      );
      return false;
    }
    ...
    const fileSizeCalc =
      typeSize.toLowerCase() === 'mb'
        ? file.size / (1024 * 1024)
        : file.size / 1024;

    if (sizeLimit && fileSizeCalc > sizeLimit) {
      validation(
        element,
        langMessages.fileSize
          .replace('{sizeLimit}', sizeLimit)
          .replace('{sizeUnit}', typeSize.toUpperCase())
          .replace('{label}', label),
      );
      return false;
    }
...
function validateFormRules(selectorForm, rules, lang = 'id') {
  let isValid = true;
  const $form = $(selectorForm);

  Object.keys(rules).forEach(selector => {
    const config = rules[selector];
    const $el =
      $form.find(selector).length > 0
        ? $form.find(selector)
        : $form.find(`[name="${selector}"]`);

    if ($el.length === 0) return;

    if (config.label) $el.attr('data-label', config.label);
    if (config.errorplace) $el.attr('data-errorplace', config.errorplace);
    ...
    if (config.rules) {
      config.rules.forEach(ruleStr => {
        ...
        const dataMap = {
          ...
          mimes: 'extfile',
          size: 'size',
          not_regex: 'specialchar',
          size_type: 'typesize',
        };

        if (dataMap[ruleName]) {
          let val = ruleValue !== undefined ? ruleValue : true;
          $el.attr(`data-${dataMap[ruleName]}`, val);
          $el.data(dataMap[ruleName], val);
        }
      });
    }
    ...
    if (typeClassMap[type]) $el.addClass(typeClassMap[type]);
  });

  const allValidations =
    '.validate-text, .validate-select, .validate-radio, .validate-file, .validate-checkbox';
  $form.find(allValidations).each(function () {
    let result = true;
    const $this = $(this);

    if ($this.hasClass('validate-text')) result = validateOnInput(this, lang);
    else if ($this.hasClass('validate-select'))
      result = validateOnSelect(this, lang);
    else if ($this.hasClass('validate-radio'))
      result = validateOnRadio($this.attr('name'), lang);
    else if ($this.hasClass('validate-file'))
      result = validateOnFile(this, lang);
    else if ($this.hasClass('validate-checkbox'))
      result = validateOnCheckbox($this.attr('name'), lang);

    if (!result) isValid = false;
  });

  initValidationOnInput({ lang: lang });
